...
This will prompt for your password, again provided by SCC. Immediately switch to sudo mode using `sudo -i`, which will prompt for your password again. Then change directory :  `cd /eprase`.
...
New images should be pulled as part of the docker 'up' process.docker
...
Service images are hosted in a private Azure registry, login details. The password is available via the Azure registry access keys page.
