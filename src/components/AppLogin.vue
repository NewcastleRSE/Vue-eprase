  ...
  <main>
    <div class="loginpage">
      ...
      <div class="alert alert-danger" v-if="!toolIsOpen">
        The ePrescribing Risk and Safety Evaluation tool (ePRaSE) is currently closed to users. Access possible for
        admins only.
      </div>
      ...
      <div v-if="toolIsOpen && multipleSessions">
        <table class="table table-striped vf-col-12">
          <thead>
            <tr>
              <th class="align-content-center" colspan="9">You have ePRaSE sessions open on other devices - please
                choose what to do</th>
            </tr>
            <tr>
              <th>Login time</th>
        ...
        <LogoutCurrentSessionModal ref="logoutCurrentSessionModal" :showActionBtn="true"
          @modal-actioned="logoutCurrentSession()" />
        <LogoutOtherSessionsModal ref="logoutOtherSessionsModal" :showActionBtn="true"
          @modal-actioned="logoutOtherSessions()" />
      </div>

      <div v-if="toolIsOpen && !multipleSessions">
        <h3 v-if="$route.query.action === 'loggedOut'" class="text-success">You have successfully logged out</h3>
        <h3 v-if="$route.query.action === 'registered'" class="text-success">Registration successful, please sign in
        </h3>
        <h3 v-if="$route.query.action === 'changedPassword'" class="text-success">Successfully changed your password,
          please sign in again</h3>
        <h3 v-if="$route.query.action === 'sessionExpired'" class="text-danger">Your session has expired or has been
          logged out from another device, please sign in again</h3>
        <h3 v-if="serverError" class="text-danger">{{ serverError }}</h3>

        <h1 class="mt-4">Log-in to ePRaSE</h1>

        <p class="pb-2">
          Please enter your login details below, or click 'Register' to create a new user account.<br>You will need a
          valid
          <span class="fw-bold">'nhs.uk'</span> or <span class="fw-bold">'nhs.net'</span> email account to register with
          ePRaSE successfully.
        </p>

        <div class="mb-4">
          <Vueform ref="loginForm" :endpoint="false" @submit="onLoginClick" v-model="user" sync>
            <TextElement name="email" placeholder="Valid NHS email address" :label="embolden('Email address', true)"
            ...
            <GroupElement name="buttonBar" :columns="12" :add-class="'mt-2'">
              ...
              <ButtonElement name="register" full :columns="3" :add-class="'mx-2'"
                :disabled="onStaging || $route.query.action === 'registered'" @click="onRegisterClick">
                <i class="bi bi-person-fill-add me-2"></i>Register
              </ButtonElement>
              <ButtonElement name="forgotpassword" full data-bs-toggle="modal" data-bs-target="#forgotPasswordModal"
                :columns="3" :add-class="'ms-2'" :disabled="$route.query.action === 'registered'">
...
export default {
  ...
  computed: {
    ...
    nonCurrentSessions() {
      return Array.isArray(this.sessions) ? this.sessions.filter(sess => (
        sess.isCurrentSession === false &&
        sess.isTrulyActive === true &&
        sess.logoutTime == null &&
        sess.minutesSinceActive < parseInt(this.jwtLifespan / 60)
      )) : []
    }
  },
  data() {
