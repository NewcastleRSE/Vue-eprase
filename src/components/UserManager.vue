                  ...
                  <tr v-for="userData in searchResults">
                    ...
                    <td>
                      ...
                      <!-- <a v-if="currentEdit.id != userData.id" type="button" class="btn btn-danger"
                        @click="onDeleteClick(userData.id)" title="Delete user"
                        :class="userData.id == currentUserId ? { 'disabled': 'disabled' } : ''">
...
export default {
  ...
  methods: {
    setCurrentEdit(userData) {
      this.currentEdit = Object.assign({}, userData, { password: '', confirmPassword: ''})
    },
    cancelCurrentEdit() {
    ...
    async deleteUser() {
      ...
      console.debug('Delete payload is', this.deleteId)

      const deleteResponse = await authenticationStore().deleteUser(this.deleteId)
